...
model_name_or_path=/mnt/afs/chenxiaoxuan/hf_home/Qwen1.5_7b # or bloomz-7b1-mt

train_file=/mnt/afs/chenxiaoxuan/BELLE/data/dataset/20240901pretrain/CLUE_1024_qwen_test.json
validation_file=/mnt/afs/chenxiaoxuan/BELLE/data/dataset/20240901pretrain/CLUE_1024_qwen_test.json
#output_dir="$ABS_PATH/BELLE/saved_models/${WANDB_PROJECT}_${WANDB_RUN_ID}"
output_dir="$ABS_PATH/BELLE/train/work_dirs/pt/test"
mkdir -p ${output_dir}
...
#!/bin/bash #找到checkpoint文件夹

# 查找匹配的目录
checkpoint_dir=$(find "$output_dir" -maxdepth 1 -type d -name "checkpoint-*")

# 提取目录名
checkpoint_name=$(basename "$checkpoint_dir")

# 输出结果
echo "找到的checkpoint文件夹路径为: $output_dir/$checkpoint_name"

#FT
torchrun --nproc_per_node 4 src/entry_point/pt_train.py \
    --ddp_timeout 36000 \
